"""
train_and_generate_3d.py
Tudo-em-um: pede a pasta com .obj (Tkinter), converte .obj -> voxel (na memória),
treina/continua um VAE3D, gera .obj na mesma pasta escolhida.

Requisitos:
pip install numpy torch trimesh scikit-image
(opcional: open3d para suavizar)
"""
...
LATENT_DIM = 1024       # capacidade do espaço latente (256-512 recomendado)
BATCH_SIZE = 12
LR = 1e-4
EPOCHS_PER_RUN = 4502    # quantas épocas treina a cada execução
SAVE_EVERY = 4000         # salva checkpoint a cada N épocas
NUM_OUTPUTS = 1         # quantos .obj gerar ao final
THRESHOLD = 0.45        # threshold para binarizar voxel antes de marching cubes
CHECKPOINT_NAME = os.path.join(PASTA_MODELOS, "vae3d_checkpoint.pth")   # arquivo .pth salvo na pasta escolhida
OUT_PREFIX = os.path.join(PASTA_MODELOS, "gerado")   # prefixo dos .obj gerados (na pasta escolhida)
VERBOSE = True
SEED = 42
...
# ========== Treina / continua a partir do checkpoint ==========
def train_and_checkpoint(model, data_tensor, device=DEVICE):
    ...
    for epoch in range(start_epoch, start_epoch + EPOCHS_PER_RUN):
        ...
        print(f"[Ep {epoch}] loss={avg_loss:.4f} BCE={avg_bce:.4f} KLD={avg_kld:.4f}")
...
# ========== Geração: samplear z ~ N(0,1) ==========
def generate_and_export(model, out_prefix=OUT_PREFIX, num_samples=NUM_OUTPUTS, device=DEVICE):
    model = model.to(device)
    model.eval()
    for i in range(num_samples):
        ...
        try:
            ...
            nome = f"{out_prefix}_{int(time.time())}_{i+1}.obj"
            ...
            try:
                ...
                mesh = vg.as_boxes()
                nome = f"{out_prefix}_boxes_{int(time.time())}_{i+1}.obj"
                mesh.export(nome)
...
def main():
    ...
    generate_and_export(model, out_prefix=OUT_PREFIX, num_samples=NUM_OUTPUTS, device=DEVICE)
