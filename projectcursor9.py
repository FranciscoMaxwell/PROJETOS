...
# ============================
# CONFIGURAÇÃO BASE (sua pasta 'Textos/cursores')
# ============================
...
def remover_pixels_quase_transparentes(img, threshold=20):
    """Zera pixels com alpha abaixo do threshold para evitar ruído de borda."""
    if img.mode != "RGBA":
...
def suavizar_borda(img, radius=1):
    """Aplica blur muito suave só na alpha para suavizar a transição."""
    # separar canais
    if img.mode != "RGBA":
        img = img.convert("RGBA")
    r, g, b, a = img.split()
...
def cortar_e_centralizar(img, tamanho_alvo):
    """
    Recorta bbox do conteúdo visível e centraliza dentro de um canvas tamanho_alvo.
    ...
    bbox = img.getbbox()
    if not bbox:
        # nada visível, retorna canvas vazio
...
def imagem_para_cur(imagem_or_path, fator_escala=3.0, sensibilidade=25, temp_dir=None):
    ...
    Gera um .cur real (a partir de ICO em memória) com hotspot 0,0.
...
def limpar_fundo(img, sensibilidade=25):
    """
    Versão preservadora: tenta limpar fundo apenas quando imagem não tem transparência parcial.
    Mantida do seu código original com pequenas garantias de segurança.
    """
...
def gif_para_curs(gif_path, fator_escala=3.0, sensibilidade=25, temp_dir=None, max_frames=None):
    """
    Converte GIF em vários .cur aplicando anti-borda em cada frame.
    Salva em subpasta única dentro de BASE_DIR e retorna (frames_list, pasta)
    """
...
def frames_para_ani(frames_list, output_ani_path, fps=10, title=None, author=None):
    """
    Monta .ani compatível a partir dos bytes dos .cur/.ico.
    """
    icons_data = []
...
# ============================
# Funções de aplicar/restaurar cursor no Windows
# ============================
...
def cur_para_ani_salvar():
    ...
    try:
        ...
        Button(win, text="🔍 Abrir pasta", width=20, command=lambda: subprocess.Popen(f'explorer /select,"{out}"')).pack(pady=5)
