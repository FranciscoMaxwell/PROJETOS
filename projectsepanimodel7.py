...
# simple material->semantic mapping (heuristic)
MATERIAL_HINTS = {
    'metal': ['metal','iron','steel','chrome','bronze'],
    'wood': ['wood','timber','oak','pine','mahogany','bark'],
    'cloth': ['cloth','fabric','cloth','cotton','silk','wool','leather'],
    'skin': ['skin','face','body','flesh'],
    'eye': ['eye','iris','pupil','glass'],
    'fur': ['fur','furry','pelt','hairy'],
}


def guess_semantic_from_material_name(name):
    ln = name.lower()
    for sem, hints in MATERIAL_HINTS.items():
        for h in hints:
            if h in ln:
                return sem
    return 'unknown'
...
try:
        ...
        try:
            ...
            if o.type == 'MESH':
                ...
                for ms in o.material_slots:
                    if not ms.material:
                        continue
                    mats_seen.add(ms.material.name)
    ...
    # Pose analysis using armature if available (existing logic)
    def vec(a):
        return a.x, a.y, a.z
    ...
    # -----------------------------
    # UV semantic mould generation (best-effort)
    # -----------------------------
    try:
        out_dir = os.path.dirname(meta_out) if meta_out else os.path.dirname(out_fbx)
        tex_dir = os.path.join(out_dir, 'texturizacao')
        mold_dir = os.path.join(tex_dir, 'moldes_uv')
        if not os.path.exists(mold_dir):
            os.makedirs(mold_dir, exist_ok=True)
        # export UV layout for each mesh object and generate semantic legend
        semantic_legend = {}
        for obj in [o for o in bpy.data.objects if o.type == 'MESH']:
            try:
                ...
                # export UV layout lines
                uv_png = os.path.join(mold_dir, f"{obj.name}_molde_uv.png")
                try:
                    bpy.ops.uv.export_layout(filepath=uv_png, export_all=True, modified=False, mode='PNG', size=(1024,1024))
                except Exception:
                    # fallback smaller
                    try:
                        bpy.ops.uv.export_layout(filepath=uv_png, export_all=True, modified=False, mode='PNG', size=(512,512))
                    except Exception:
                        pass
                # detect materials per object and create semantic mapping
                mats = [ms.material.name if ms.material else 'None' for ms in obj.material_slots]
                sems = []
                for mname in mats:
                    if mname and mname != 'None':
                        sem = guess_semantic_from_material_name(mname)
                        sems.append(sem)
                        semantic_legend[sem] = SEMANTIC_COLORS.get(sem, SEMANTIC_COLORS['unknown'])
                # also if particle systems detected -> fur
                if getattr(obj, 'particle_systems', None) and len(obj.particle_systems) > 0:
                    semantic_legend['fur'] = SEMANTIC_COLORS['fur']
                # create placeholder mask images per semantic class (solid color under UV layout)
                # NOTE: accurate masked textures require baking or more advanced UV->pixel mapping; here we create
                # semantic mask placeholders to be used by downstream tools and a legend JSON.
                for sem in set(sems):
                    mask_path = os.path.join(mold_dir, f"{obj.name}_mask_{sem}.png")
                    # create a new image and fill with the semantic color (placeholder)
                    try:
                        img = bpy.data.images.new(name=f"mask_{obj.name}_{sem}", width=1024, height=1024, alpha=True, float_buffer=False)
                        r,g,b = [c/255.0 for c in SEMANTIC_COLORS.get(sem, SEMANTIC_COLORS['unknown'])]
                        pixels = [r,g,b,1.0] * (1024*1024)
                        img.pixels = pixels
                        img.filepath_raw = mask_path
                        img.file_format = 'PNG'
                        img.save()
                        # remove image from memory to avoid bloating
                        bpy.data.images.remove(img)
                    except Exception:
                        pass
        # write legend json
        legend_path = os.path.join(mold_dir, 'molde_legend.json')
        try:
            with open(legend_path, 'w', encoding='utf-8') as lf:
                json.dump(semantic_legend, lf)
        except Exception:
            pass
    except Exception:
        pass
