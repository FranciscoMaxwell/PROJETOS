...
class SalesModule(ModuleFrame):
    def __init__(self, master, app):
        super().__init__(master, app, title="Sales Cloud — Vendas")
        self.build_ui()
        self.refresh()
    ...
    def build_ui(self):
        ...
        pan.add(left, weight=1); pan.add(right, weight=3)

        # Leads
        tb.Label(left, text="Leads", font=('Inter',12,'bold')).pack(anchor='w')
        self.lead_list = tk.Listbox(left, height=16)
        self.lead_list.pack(fill='both', expand=True, pady=(6,4))
        lbtnf = tb.Frame(left); lbtnf.pack(fill='x', pady=6)
        tb.Button(lbtnf, text="Novo", bootstyle="success-outline", command=self.new_lead).pack(side='left', expand=True, padx=4)
        tb.Button(lbtnf, text="Converter", bootstyle="primary-outline", command=self.convert_lead).pack(side='left', expand=True, padx=4)
        tb.Button(lbtnf, text="Deletar", bootstyle="danger-outline", command=self.delete_lead).pack(side='left', expand=True, padx=4)

        # Contacts (right top)
        cont_frame = tb.Labelframe(right, text="Contatos", padding=6); cont_frame.pack(fill='both', expand=True, pady=4)
        import tkinter.ttk as ttk
        cols = ('id','name','email','phone','account','tags')
        self.contact_tree = ttk.Treeview(cont_frame, columns=cols, show='headings', height=8)
        for c in cols: self.contact_tree.heading(c, text=c)
        self.contact_tree.pack(fill='both', expand=True)
        cbtnf = tb.Frame(cont_frame); cbtnf.pack(fill='x', pady=6)
        tb.Button(cbtnf, text="Novo", bootstyle="success", command=self.new_contact).pack(side='left', padx=4)
        tb.Button(cbtnf, text="Editar", bootstyle="warning", command=self.edit_contact).pack(side='left', padx=4)
        tb.Button(cbtnf, text="Import CSV", bootstyle="info", command=self.import_contacts_csv).pack(side='left', padx=4)
        tb.Button(cbtnf, text="Export CSV", bootstyle="secondary", command=self.export_contacts_csv).pack(side='left', padx=4)

        # Opportunities (right bottom)
        opp_frame = tb.Labelframe(right, text="Oportunidades", padding=6); opp_frame.pack(fill='both', expand=True, pady=4)
        cols2 = ('id','title','contact','amount','stage','close_date')
        self.op_tree = ttk.Treeview(opp_frame, columns=cols2, show='headings', height=8)
        for c in cols2: self.op_tree.heading(c, text=c)
        self.op_tree.pack(fill='both', expand=True)
        obtnf = tb.Frame(opp_frame); obtnf.pack(fill='x', pady=6)
        tb.Button(obtnf, text="Nova", bootstyle="success", command=self.new_opportunity).pack(side='left', padx=4)
        tb.Button(obtnf, text="Editar", bootstyle="warning", command=self.edit_opportunity).pack(side='left', padx=4)
        tb.Button(obtnf, text="Set → Ganho", bootstyle="success-outline", command=lambda: self.set_stage('ganho')).pack(side='left', padx=4)
        tb.Button(obtnf, text="Set → Perdido", bootstyle="danger-outline", command=lambda: self.set_stage('perdido')).pack(side='left', padx=4)

    def refresh(self):
        # leads
        self.lead_list.delete(0,'end')
        rows = db_fetch("SELECT id,name,email,status FROM leads ORDER BY created_at DESC")
        for r in rows: self.lead_list.insert('end', f"{r[0]} | {r[1]} | {r[2]} | {r[3]}")
        # contacts
        for i in list(self.contact_tree.get_children()): self.contact_tree.delete(i)
        rows = db_fetch("SELECT id,name,email,phone,account,tags FROM contacts ORDER BY created_at DESC")
        for r in rows: self.contact_tree.insert('', 'end', values=r)
        # opportunities
        for i in list(self.op_tree.get_children()): self.op_tree.delete(i)
        rows = db_fetch("""SELECT o.id,o.title,c.name,o.amount,o.stage,o.close_date
                           FROM opportunities o LEFT JOIN contacts c ON o.contact_id=c.id ORDER BY o.created_at DESC""")
        for r in rows: self.op_tree.insert('', 'end', values=r)

    # Leads
    def new_lead(self):
        dlg = LeadDialog(self); self.wait_window(dlg)
        if getattr(dlg,'result',None):
            name,email,phone,source = dlg.result; now=datetime.datetime.now().isoformat()
            db_execute("INSERT INTO leads (name,email,phone,source,status,created_at) VALUES (?,?,?,?,?,?)", (name,email,phone,source,'new',now)); self.refresh()
    def delete_lead(self):
        sel = self.lead_list.curselection()
        if not sel: messagebox.showwarning("Atenção","Selecione um lead"); return
        text = self.lead_list.get(sel[0]); lead_id = int(text.split('|')[0].strip())
        if messagebox.askyesno("Confirmar","Deletar lead?"):
            db_execute("DELETE FROM leads WHERE id=?", (lead_id,)); self.refresh()
    def convert_lead(self):
        sel = self.lead_list.curselection()
        if not sel: messagebox.showwarning("Atenção","Selecione um lead"); return
        text = self.lead_list.get(sel[0]); lead_id = int(text.split('|')[0].strip())
        row = db_fetch("SELECT name,email,phone FROM leads WHERE id=?", (lead_id,))
        if not row: return
        name,email,phone = row[0]; now = datetime.datetime.now().isoformat()
        db_execute("INSERT INTO contacts (name,email,phone,account,tags,created_at) VALUES (?,?,?,?,?,?)", (name,email,phone,'','converted',now))
        db_execute("UPDATE leads SET status='converted' WHERE id=?", (lead_id,)); messagebox.showinfo("Convertido", f"Lead '{name}' convertido."); self.refresh()

    # Contacts
    def new_contact(self):
        dlg = ContactDialog(self); self.wait_window(dlg)
        if getattr(dlg,'result',None):
            name,email,phone,account,tags = dlg.result; now=datetime.datetime.now().isoformat()
            db_execute("INSERT INTO contacts (name,email,phone,account,tags,created_at) VALUES (?,?,?,?,?,?)", (name,email,phone,account,tags,now)); self.refresh()
    def edit_contact(self):
        sel = self.contact_tree.selection()
        if not sel: messagebox.showwarning("Atenção","Selecione um contato"); return
        item = self.contact_tree.item(sel[0])['values']; dlg = ContactDialog(self, prefill=item); self.wait_window(dlg)
        if getattr(dlg,'result',None):
            name,email,phone,account,tags = dlg.result; cid=item[0]
            db_execute("UPDATE contacts SET name=?,email=?,phone=?,account=?,tags=? WHERE id=?", (name,email,phone,account,tags,cid)); self.refresh()
    def import_contacts_csv(self):
        path = filedialog.askopenfilename(filetypes=[("CSV files","*.csv")])
        if not path: return
        try: df = pd.read_csv(path)
        except Exception as e: messagebox.showerror("Erro", f"Falha ao ler CSV: {e}"); return
        now = datetime.datetime.now().isoformat(); rows=[]
        for _,r in df.iterrows(): rows.append((r.get('name',''), r.get('email',''), r.get('phone',''), r.get('account',''), r.get('tags',''), now))
        if rows: db_execute_many("INSERT INTO contacts (name,email,phone,account,tags,created_at) VALUES (?,?,?,?,?,?)", rows); messagebox.showinfo("Importado", f"{len(rows)} contatos importados."); self.refresh()
    def export_contacts_csv(self):
        path = filedialog.asksaveasfilename(defaultextension='.csv', filetypes=[('CSV','*.csv')]); 
        if not path: return
        rows = db_fetch("SELECT name,email,phone,account,tags FROM contacts")
        with open(path,'w', newline='', encoding='utf-8') as f:
            w = csv.writer(f); w.writerow(['name','email','phone','account','tags']); w.writerows(rows)
        messagebox.showinfo("Exportado", f"Contacts saved to {path}")

    # Opportunities
    def new_opportunity(self):
        dlg = OpportunityDialog(self); self.wait_window(dlg)
        if getattr(dlg,'result',None):
            title,contact_id,amount,stage,close_date = dlg.result; now=datetime.datetime.now().isoformat()
            db_execute("INSERT INTO opportunities (title,contact_id,amount,stage,close_date,created_at) VALUES (?,?,?,?,?,?)", (title,contact_id,amount,stage,close_date,now)); self.refresh()
    def edit_opportunity(self):
        sel = self.op_tree.selection()
        if not sel: messagebox.showwarning("Atenção","Selecione uma oportunidade"); return
        item = self.op_tree.item(sel[0])['values']
        contact_name = item[2]; contact_id=None
        if contact_name:
            row = db_fetch("SELECT id FROM contacts WHERE name=?", (contact_name,))
            if row: contact_id = row[0][0]
        prefill = (item[1], contact_id or '', item[3], item[4], item[5])
        dlg = OpportunityDialog(self, prefill=(item[0],)+prefill); self.wait_window(dlg)
        if getattr(dlg,'result',None):
            title, contact_id, amount, stage, close_date = dlg.result; oid=item[0]
            db_execute("UPDATE opportunities SET title=?,contact_id=?,amount=?,stage=?,close_date=? WHERE id=?", (title,contact_id,amount,stage,close_date,oid)); self.refresh()
    def set_stage(self, stage_value):
        sel = self.op_tree.selection()
        if not sel: messagebox.showwarning("Atenção","Selecione uma oportunidade"); return
        oid = self.op_tree.item(sel[0])['values'][0]; db_execute("UPDATE opportunities SET stage=? WHERE id=?", (stage_value, oid)); self.refresh()
...
class CRMApp(tb.Window):
    ...
    def _build_sidebar(self):
        tb.Label(self.sidebar, text=" Mini CRM ", font=('Inter',18,'bold')).pack(pady=(12,6))
...
class CommerceModule(ttk.Frame):
    ...
    def del_produto(self):
        ...
        if not sel:
            ...
            return
        pid = self.tree_prod.item(sel[0])["values"][0]
        db_execute("DELETE FROM produtos WHERE id=?", (pid,))
        self.load_produtos()
